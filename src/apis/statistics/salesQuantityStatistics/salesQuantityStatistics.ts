...
import { GET_BEST_SELLERS_BY_CATEGORY, GET_MONTHLY_BEST_SELLERS, GET_SALES_QUANTITY_BY_BRANCH, GET_TOP_100_BEST_SELLERS, GET_WEEKLY_BEST_SELLERS, GET_YEARLY_BEST_SELLERS } from "@/apis/constants/jsj.constants";
...
// 판매량 통계
// 지점별
export const getSalesQuantityByBranch = async(accessToken: string): Promise<ResponseDto<SalesQuantityStatisticsReponseDto[]>> => {
  try{
    const response = await axiosInstance.get(GET_SALES_QUANTITY_BY_BRANCH, bearerAuthorization(accessToken));
    return responseSuccessHandler(response);
  }catch(error){
    return responseErrorHandler(error as AxiosError<ResponseDto>)
  }
